                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <c:forEach items="${adminList}" var="admin">
                                <tr>
                                    <td>${admin.usId}</td>
                                    <td>${admin.usName}</td>
                                    <td>${admin.usPassword}</td>
                                    <td>${admin.usSex}</td>
                                    <td>${admin.rdType.rdTypeName}</td>
                                    <td>${admin.usDepartment}</td>
                                    <td>${admin.usPhone}</td>
                                    <td>${admin.usEmail}</td>
                                    <td><fmt:formatDate value="${admin.usDateRegister}" pattern="yyyy-MM-dd"/></td>
                                    <td>${admin.usBorrowCount}</td>
                                    <td>${admin.usStatus}</td>
                                    <td>${admin.adRole.adRoleName}</td>
                                    <td><img src="${pageContext.request.contextPath}/user_showImg.action?usId=${admin.usId}" alt="图片" width="30" height="30"></td>
                                    ...
                                    <td><a href="${pageContext.request.contextPath}/user_deleteAdmin.action?usId=${admin.usId}">删除</a> <a href="${pageContext.request.contextPath}/user_toEditAdminPage.action?usId=${admin.usId}">修改</a></td>
                                </tr>
                            </c:forEach>
                            </tbody>
                        </table>
