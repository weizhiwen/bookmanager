        ...
        <div class="main-right col-lg-10 col-md-10 col-sm-9">
            <div class="row">
                <ul class="breadcrumb">
                    <span>当前位置：</span>
                    <li><a href="#">读者管理</a></li>
                    <li class="active">修改读者类型</li>
                </ul>
            </div>
            <div class="panel panel-default">
                <div class="panel-body">
                    <form action="${pageContext.request.contextPath}/rdType_edit.action" method="post">
                        <div class="form-inline">
                            <div class="form-group input-group-sm">
                                <label for="id">读者类别编号：</label>
                                <input type="number"  id="id" class="form-control" name="rdTypeId" value="${rdType.rdTypeId}">
                            </div>&nbsp;
                            <div class="form-group input-group-sm">
                                <label for="name">读者类别名称：</label>
                                <input type="text"  id="name" class="form-control" name="rdTypeName" value="${rdType.rdTypeName}">
                            </div>&nbsp;
                            <div class="form-group input-group-sm">
                                <label for="count">可借书数量：</label>
                                <input type="number" id="count" class="form-control"  name="canLendCount" value="${rdType.canLendCount}">
                            </div>&nbsp;
                            <div class="form-group input-group-sm">
                                <label for="day">可借书天数：</label>
                                <input type="number" id="day" class="form-control"  name="canLendDay" value="${rdType.canLendDay}">
                            </div>
                        </div><br />
                        <div class="form-inline">
                            <div class="form-group input-group-sm">
                                <label for="time">可续借的次数：</label>
                                <input type="number" id="time" class="form-control"  name="canCountTime" value="${rdType.canCountTime}">
                            </div>
                            <div class="form-group input-group-sm">
                                <label for="rate">罚款率：</label>
                                <input type="number" id="rate" class="form-control"  name="fineRate" value="${rdType.fineRate}">
                            </div>&nbsp;
                            <div class="form-group input-group-sm">
                                <label for="year">证书有效期（年）：</label>
                                <input type="text" id="year" class="form-control" name="dateValid" value="${rdType.dateValid}">
                            </div>
                        </div><br />
